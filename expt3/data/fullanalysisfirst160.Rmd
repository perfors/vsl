---
title: "Statistical Learning Experiment: first 160 participants"
output:
...
load("alldata160.RData")
```

# Summary of datasets

This reports all the same analyses just with the first 160 participants, since that's what we preregistered.
...
One interesting thing that I wasn't expecting is that it looks like Chinese people actually do *worse* on the Unfamiliar stimuli -- in particular, nobody does well, the distribution cuts off at around 80% accuracy. I checked to see if there was a significant difference between Chinese people on the Unfamiliar stimuli and English people on them, and indeed there is: language A: t=`r round(tAunf[[1]]["t"][[1]],3)`, df=`r round(tAunf[[2]]["df"][[1]],3)`, p=`r round(tAunf[[3]],3)`; language B: t=`r round(tBunf[[1]]["t"][[1]],3)`, df=`r round(tBunf[[2]]["df"][[1]],3)`, p=`r round(tBunf[[3]],3)`. My post-hoc explanation for this is that it may be a kind of interference, the visual analogue of the phonological interference identified in previous studies: the unfamiliar stimuli are visually similar enough to their overlearned Chinese characters that they're actually having a harder time processing them than English speakers are. But this is post-hoc obviously.
...
\vspace{16pt}
```{r, echo=FALSE, message=FALSE, fig.height=3, fig.width=8, fig.align='center'}
...
\vspace{16pt}
```{r, echo=FALSE, message=FALSE, fig.height=3, fig.width=9, fig.align='center'}
...
We can also break the **same** categories into Familiar and Unfamiliar, split between Chinese speakers and English speakers. We'd expect the correlation for English speakers to be the same for Familiar and Unfamiliar, but Chinese speakers to be more correlated for Unfamiliar stimuli (if they act like the participants in Experiment 1). 

```{r, echo=FALSE, warning=FALSE, fig.height=5, fig.width=9, fig.align='center'}
p1 <- ggscatter(chA, x="SLcorrectChinese", y="PFavgChinese", add="reg.line", cor.coef=TRUE, 
                cor.method = "pearson", xlab = "SL Chinese", ylab="PF Chinese", size=2, 
                cor.coef.coord=c(0.6,0.46)) +
  ggtitle("Chinese people, Chinese stimuli") + theme(plot.title = element_text(hjust=0.5))
p2 <- ggscatter(chA, x="SLcorrectUnfamiliar", y="PFavgUnfamiliar", add="reg.line", cor.coef=TRUE, 
                cor.method = "pearson", xlab = "SL Unfamiliar", ylab="PF Unfamiliar", size=2, 
                cor.coef.coord=c(0.6,0.46)) +
  ggtitle("Chinese people, Unfamiliar stimuli") + theme(plot.title = element_text(hjust=0.5))

p3 <- ggscatter(engA, x="SLcorrectChinese", y="PFavgChinese", add="reg.line", cor.coef=TRUE, 
                cor.method = "pearson", xlab = "SL Chinese", ylab="PF Chinese", size=2, 
                cor.coef.coord=c(0.6,0.46)) +
  ggtitle("English people, Chinese stimuli") + theme(plot.title = element_text(hjust=0.5))
p4 <- ggscatter(engA, x="SLcorrectUnfamiliar", y="PFavgUnfamiliar", add="reg.line", cor.coef=TRUE, 
                cor.method = "pearson", xlab = "SL Unfamiliar", ylab="PF Unfamiliar", size=2, 
                cor.coef.coord=c(0.6,0.46)) +
  ggtitle("English people, Unfamiliar Stimuli") + theme(plot.title = element_text(hjust=0.5))
